...
def compute_selectivity(obp_results, interferents, voc_diana_smiles, 
    ...
    # Carrega el CSV d'iobpdb per consultar dades experimentals
    df_iobpdb = pd.read_csv(iobpdb_path)

    # Resol SMILES de cada interferent via PubChem
    ...
    for obp in obp_results:
        ...
        for interf in interferent_data:
            interf_name  = interf["name"]
            interf_slug  = interf_name.replace(" ", "_").replace("/", "-")
            ligand_pdbqt = f"docking/interferents/{interf_slug}.pdbqt"
            out_pdbqt    = f"docking/interferents/{name}_{interf_slug}_out.pdbqt"

            # ── Busca primer al CSV iobpdb ──────────────────
            kd_exp = None
            mask = df_iobpdb["Compound name"].str.contains(
                interf_name, case=False, na=False)
            row = df_iobpdb[mask]

            if not row.empty and name in df_iobpdb.columns:
                val = row.iloc[0][name]
                if not pd.isna(val):
                    val_str = str(val).strip()
                    if not val_str.startswith(">"):
                        try:
                            kd_exp = int(round(float(val_str) * 1000))
                        except:
                            pass

            if kd_exp is not None:
                # Usa el Kd experimental del CSV
                kd_interferents.append(kd_exp)
                print(f"    {interf_name:20} Kd exp: {kd_exp} nM  [iobpdb]")
                continue

            # ── Si no hi ha dada experimental, fa docking ──
            if not Path(ligand_pdbqt).exists():
                try:
                    prepare_ligand(interf["smiles"], ligand_pdbqt)
                except Exception as e:
                    print(f"    ⚠ Error preparant {interf_slug}: {e}")
                    continue
            ...
            if score_interf is None:
                print(f"    {interf_name:20} INVÀLID (saltat)")
                continue

            kd_interf_nm = score_to_kd_nm(score_interf)
            kd_interferents.append(kd_interf_nm)
            print(f"    {interf_name:20} score: {score_interf:.3f} kcal/mol "
                  f"| Kd pred: {kd_interf_nm:.0f} nM  [docking]")
