...
# ─── CONFIGURACIÓ ────────────────────────────────────────────
VOC_NAME         = "linalool"   # canvia aquí el VOC que vulguis
TOP_N_DOCKING    = 5
...
def is_mutant(obp_name: str) -> bool:
    """
    Detecta si un OBP és una mutant de laboratori.
    Les mutants tenen un espai seguit d'un patró tipus P99A, H29Y, E94V...
    (lletra majúscula + número + lletra majúscula)
    """
    import re
...
def fase_a(voc_name):
    ...
    # Filtra mutants artificials
    before = len(results)
    results = [r for r in results if not is_mutant(r["obp_name"])]
    filtered = before - len(results)
    if filtered > 0:
        print(f"  Eliminades {filtered} mutants artificials del ranking")


    results = sorted(results, key=lambda x: x["kd_nm"])
    print(f"  Trobades {len(results)} OBPs naturals amb Kd mesurat")
    results = sorted(results, key=lambda x: x["kd_nm"])
...
def download_iobpdb_structure(obp_name: str, out_pdb: str) -> bool:
    """Descarrega l'estructura PDB directament del repositori iobpdb."""
    url = IOBPDB_PDB_URL.format(obp_name)
    try:
        r = requests.get(url, timeout=30)
        if r.status_code == 200:
            with open(out_pdb, "w") as f:
                f.write(r.text)
            print(f"    ✓ Estructura descarregada d'iobpdb")
            return True
        else:
            print(f"    ⚠ No trobat a iobpdb (status {r.status_code})")
            return False
    except Exception as e:
...
def get_uniprot_id_uniprot(obp_name: str):
    """Busca l'UniProt ID automàticament com a últim recurs."""
    try:
        r = requests.get(
...
def fase_c(obp_results, voc_smiles):
    ...
    for obp in top_obps:
        ...
        if not os.path.exists(pdb_path):
            print(f"    Buscant estructura a iobpdb...")
            if not download_iobpdb_structure(name, pdb_path):
                print(f"    Intentant ESMFold...")
                uid = get_uniprot_id_uniprot(name)
                if not uid:
                ...
                if not predict_structure(seq, pdb_path):
                    ...
                    continue
                print(f"    ✓ Estructura predita per ESMFold")
